...
|---|---|---|---|---|---|---|---|---|---|---|---|
| front-page | Unemployment by Locality | `map-cell` | Unemployment Rate by Locality | choropleth map | `employment_by_locality.json` → `.counties[].{hc_key,unemployed_rate,labor_force,employed,region}` | LMD-RUN Q1 | `LABORFORCE` UnemployedRate/Employed/LaborForce (AreaType=04, PeriodType=03, Adjusted=0) + GEOGRAPHIES/SUBGEOGRAPHIES for region | county/city (133) | monthly | null rate → gray fill; NSA at county | real (deployed file predates v8 labels — see churn flags) |
...
| front-page | Unemployment Rate Trending | `line-cell` | Unemployment Rate Trending | line | `unemployment_trend.json` → `.months`, `.series.virginia`, `.series.us_national`, `.counties[].data` | LMD-RUN Q2 | `LABORFORCE` monthly UnemployedRate (VA 01/Adj1; US aggregate; county 04/Adj0; PeriodType=03) | VA + US + per-county | monthly (36-mo window) | `null` at 2025-10; `connectNulls:false` | representative (values read as smoothed/curated, not raw LAUS) |
| front-page | Jobs Added by Industry | `bar-cell` | Jobs Added by Industry | horizontal bar | `jobs_by_industry.json` → `.statewide[]` / `.regions[].sectors[]` `{sector,jobs_added}` | LMD-RUN Q3 | `INDUSTRY` QuarterAvgEmp (PeriodType=02); private supersectors IndCode 1011–1027 Own=50 + Gov rollup; labels `NAICSSuperSectors.SuperTitle`; `jobs_added` = LAG(qtr) delta; top-5 by ROW_NUMBER | statewide + 14 LWDA | quarterly | negative bars styled; COALESCE→0 | representative (deployed predates v8: stale labels, no Gov statewide bar) |
...
| community-profiles | 03 Labor Force | `c-unemp-trend` | Unemployment Rate | line (region vs VA dashed) | `unemployment_trend.json` (`months`,`series.virginia`,`counties[us-va-<FIPS3>].data`); else `d.unemployment` | LMD-RUN Q2 | `LaborForce` monthly UnemployedRate (NSA county, VA series); `hc_key='us-va-'+RIGHT(Area,3)` | **County/City only** (real); other levels → mock | monthly | LAUS not suppression-limited | real (County/City); mock elsewhere |
...
{ months: [str "2023-04", … "2026-03"],           // 36
  ...
  counties:[ { hc_key:str "us-va-001", data:[num|null] /* NSA monthly rate, 36 */ } ] }  // 133
}
```
...
{ as_of: str "2026-03",
  ...
  counties:[ { hc_key:str "us-va-001", areaname:str, region:str "000456" /* 6-digit LWDA */,
...
1. **Front-page 3 JSONs predate the v8 SQL label contract.** Field structure matches LMD-RUN v8, but label *values* are the retired hardcoded short forms: `lwda_short_name` = `"Hampton Roads"` (v8 emits `GEOGRAPHIES.AreaName` verbatim → `"Hampton Roads (LWDA XIV)"`); `regions[].label` short vs verbose; `sectors[].sector` = `"Trade & Transportation"` (v8 sources `NAICSSuperSectors.SuperTitle` → `"Trade, Transportation and Utilities"`). Statewide bar is also **missing the Government rollup** v8 unions in. Conclusion: **the deployed front-page files are representative v3-era artifacts, not RUN_v8 output.** Every LWDA label and sector string will churn on first real run, and the front-end renders these raw in tooltips/legends — so it needs render-time abbreviation or the labels get long.
...
3. **`hc_key` round-trip.** SQL emits `'us-va-'+RIGHT(Area,3)`; the browser converts it *back* to 5-digit FIPS to join the GeoJSON. Pure convenience churn — the GeoJSON join wants FIPS, which the SQL already has as `Area`. Flagged "Assumed—pending validation" in the SQL header.
